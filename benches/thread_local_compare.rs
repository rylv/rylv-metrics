...
fn benchmark_count_add_tls_compare(c: &mut Criterion) {
    ...
    group.bench_function("count_add_tls_on", |b| {
        ...
        let mut tags = [
            ...
            RylvTag::from(RylvStr::from_static("kind:count")),
        ];
        b.iter(|| {
            collector.count_add(
...
fn benchmark_histogram_tls_compare(c: &mut Criterion) {
    ...
    group.bench_function("histogram_tls_on", |b| {
        ...
        let mut tags = [
            RylvTag::from(RylvStr::from_static("env:bench")),
            RylvTag::from(RylvStr::from_static("kind:histogram")),
        ];
        b.iter(|| {
            collector.histogram(
    ...
    std::thread::scope(|scope| {
        for index in 0..thread_count {
            ...
            scope.spawn(move || {
                let mut tags = [
                    RylvTag::from(RylvStr::from_static("env:bench")),
                    RylvTag::from(RylvStr::from_static("kind:parallel")),
                ];
                for _ in 0..work {
                    collector.count_add(
    ...
    std::thread::scope(|scope| {
        for index in 0..thread_count {
            ...
            scope.spawn(move || {
                let mut tags = [
                    RylvTag::from(RylvStr::from_static("env:bench")),
                    RylvTag::from(RylvStr::from_static("kind:parallel-h")),
                ];
                for _ in 0..work {
                    collector.histogram(
