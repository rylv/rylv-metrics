...
fn test_collector_survives_unreachable_destination() {
    ...
    for i in 0..100 {
        collector.count(
            ...
            &mut [RylvTag::from_static("scenario:unreachable")],
        );
        collector.histogram(
...
fn test_collector_survives_destination_closed_midstream() {
    ...
    collector.count(
        ...
        &mut [RylvTag::from_static("phase:before")],
    );
    ...
    for _ in 0..50 {
        collector.count(
            RylvStr::from_static("midstream.count"),
            &mut [RylvTag::from_static("phase:after")],
        );
    }
...
fn test_graceful_shutdown_flushes_pending_metrics() {
    ...
    collector.count(
        ...
        &mut [RylvTag::from_static("test:flush")],
    );
...
fn test_shared_collector_consecutive_drains_reset_state() {
    ...
    collector.count(
        ...
        &mut [RylvTag::from_static("round:1")],
    );
    ...
    collector.count(
        RylvStr::from_static("consecutive.count"),
        &mut [RylvTag::from_static("round:2")],
    );
